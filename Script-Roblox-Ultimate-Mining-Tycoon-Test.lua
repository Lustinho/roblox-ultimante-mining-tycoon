...
local function ETDSH_fake_script() -- resetc4button.mobile reset c4 button 
	...
	local function resetC4()
		...
		task.wait(3) -- Wait 3 seconds to ensure mobile controls load
		...
		if newRoot and newHumanoid then
			-- Restore position and walkspeed
			newRoot.CFrame = savedCFrame
			newHumanoid.WalkSpeed = savedWalkSpeed
			
			-- Fix camera to prevent zoom
			camera.CameraType = Enum.CameraType.Custom
			camera.CameraSubject = newHumanoid
			camera.CFrame = savedCameraCFrame
			
			-- Ensure camera stays fixed for 3 seconds
			local cameraFixConnection
			local fixAttempts = 0
			local maxFixAttempts = 90 -- ~3 seconds at 30 FPS
			...
			warn("Failed to restore character after reset!")
		end
	end
